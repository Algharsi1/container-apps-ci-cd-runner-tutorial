...
cleanup() {
 ...
 if [ -e config.sh ]; then
   ...
   while true; do
     ./config.sh remove --unattended --auth SP --clientid "$SERVICE-PRINCIPAL-CLIENT-ID" --tenantid "$SERVICE-PRINCIPAL-TENANT-ID" --clientsecret "$SERVICE-PRINCIPAL-CLIENT-SECRET" && break
